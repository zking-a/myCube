 ...
 * 棋盘、轮次与胜负全部由 /gomoku-ws 服务端下发。
 */
(function exposeGomokuNet(root, factory) {
  const api = factory();
  if (typeof module === 'object' && module.exports) module.exports = api;
  if (root) root.GomokuNet = api;
})(typeof globalThis !== 'undefined' ? globalThis : this, function createGomokuNetApi() {
  const ROOM_RE = /^[A-HJ-NP-Z2-9]{5}$/;
  const SESSION_PREFIX = 'gomoku_online_session_v1_';
  ...
  function websocketUrl(locationLike) {
    ...
    return (source.protocol === 'https:' ? 'wss://' : 'ws://') + source.host + '/gomoku-ws';
  }
